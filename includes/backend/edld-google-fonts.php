<?php
  ...
  if(!empty($font_json)){
    ...
    foreach($font_decoded->items as $font_details){
      $edld_font ='';
      $font_family = $font_details->family;
      $edld_font.=$font_family.'=';
      foreach($font_details->subsets as $subset){
        $add_subset = $edld_font.$subset.'=';
        foreach($font_details->variants as $variant){
          $last_result[] = $add_subset.$variant;
        }
      } 
    }
    if(!empty($last_result)){
      update_option('edld_font_list',$last_result);  
    }
  }
?>